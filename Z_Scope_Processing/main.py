...
def process_flight_batch(
    ...
    """
    Batch process all .tiff files in a flight directory, using the provided navigation file.
    ...
    for idx, tiff_path in enumerate(tiff_files):
        print(f"\nProcessing file {idx + 1}/{len(tiff_files)}: {tiff_path}")
...
def run_processing():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to merged navigation CSV (e.g., merged_103_nav.csv) for x-axis labeling in batch mode.",
    )
