...
def enhanced_cbd_selection_with_validation(image_full, base_filename):
    """Enhanced CBD selection with 3-point validation."""
    from .interactive_tools import ThreePointSelector

    print("Starting ENHANCED 3-point CBD tick mark selection...")
    print(
        "This version automatically determines 13 vs 14 tick marks based on spacing validation."
    )

    # Use new 3-point selector from interactive_tools
    selector = ThreePointSelector(
        image_full, title=f"3-Point CBD Selection: {base_filename}"
    )

    if len(selector.selected_points) != 3:
        print("ERROR: CBD selection failed - need exactly 3 points")
        return None, None

    # Calculate tick positions and CBD sequence
    calculated_ticks, tick_count, cbd_sequence = (
        selector.calculate_tick_count_and_positions(base_filename)
    )

    if calculated_ticks is None:
        print("ERROR: Could not calculate tick positions")
        return None, None

    print(f"SUCCESS: 3-point CBD selection completed")
    print(f"INFO: Detected {tick_count} tick marks")
    print(f"INFO: CBD range: {cbd_sequence[0]} to {cbd_sequence[-1]}")

    return calculated_ticks, cbd_sequence
...
def plot_with_gaps(ax, x, y, *args, **kwargs):
    """
    Plot 1D array y (as a function of x) as discontinuous segments at NaN gaps.
    """
    # Identify indices that are not NaN
    valid = np.isfinite(y)
    ...
    # np.diff(valid) will be True at transitions between True and False
    segments = []
    idx = np.arange(len(y))
    in_segment = False
    ...
    for k, is_valid in enumerate(valid):
        if is_valid and not in_segment:
            start = k
            in_segment = True
        elif (not is_valid or k == len(y) - 1) and in_segment:
            end = k if not is_valid else k + 1  # include last index if valid
            segments.append((start, end))
            in_segment = False

    # Plot each valid segment individually
    for s, e in segments:
        ax.plot(x[s:e], y[s:e], *args, **kwargs)
...
def create_time_calibrated_zscope(
    ...
    # --- ENHANCED CBD TICK MARK SELECTION WITH 3-POINT VALIDATION ---
    detection_method = output_params.get("cbd_detection_method", "enhanced_3point")
    use_manual_cbd_selection = detection_method in ["manual", "enhanced_3point"]

    if use_manual_cbd_selection:
        if detection_method == "enhanced_3point":
            print(
                "Using ENHANCED 3-point CBD tick mark selection with automatic count detection..."
            )
            calculated_ticks, cbd_sequence = enhanced_cbd_selection_with_validation(
                image_full, base_filename
            )

            if calculated_ticks is None:
                print("ERROR: Enhanced 3-point CBD selection failed")
                return None, None, None

            # Store both ticks and sequence in processor
            if processor_ref:
                processor_ref.calculated_ticks = calculated_ticks
                processor_ref.cbd_sequence = cbd_sequence

            cbd_tick_xs = calculated_ticks

        else:  # detection_method == "manual"
            print(
                "Using manual CBD tick mark selection with local image recognition..."
            )
            cbd_tick_xs = manual_cbd_tick_selection(
                image_full,
                expected_count=13,
                sprocket_removal_ratio=output_params.get(
                    "sprocket_removal_ratio", 0.08
                ),
    ...
    # *** CRITICAL FIX: Load navigation data BEFORE creating visualization ***
    if nav_df is None and nav_path is not None:
        try:
            nav_df = pd.read_csv(nav_path)
            print(
                f"INFO: Loaded navigation data with {len(nav_df)} records for visualization"
            )
        ...
        if flight_match:
            ...
            if Path(nav_file_guess).exists():
                try:
                    ...
                    print(
                        f"INFO: Auto-loaded navigation data from {nav_file_guess} with {len(nav_df)} records"
                    )
                except Exception as e:
                    print(f"WARNING: Could not auto-load navigation file: {e}")

    # === ENHANCED X-AXIS LABELS WITH CBD COORDINATES ===
    if nav_df is not None and len(cbd_tick_xs) > 0:
        ...
        if cbd_match:
            ...
            # Use validated CBD sequence if available from 3-point selection
            if hasattr(processor_ref, "cbd_sequence") and processor_ref.cbd_sequence:
                cbd_list = processor_ref.cbd_sequence
                print(
                    f"INFO: Using validated CBD sequence from 3-point selection: {len(cbd_list)} CBDs"
                )
            else:
                ...
                print(f"INFO: Created CBD sequence from filename: {len(cbd_list)} CBDs")

            # Adjust CBD list to match detected ticks
            if len(cbd_tick_xs) != len(cbd_list):
                print(
                    f"INFO: Adjusting CBD list to match {len(cbd_tick_xs)} detected ticks"
                )
                if len(cbd_list) > len(cbd_tick_xs):
                    cbd_list = cbd_list[: len(cbd_tick_xs)]
                elif len(cbd_list) < len(cbd_tick_xs):
                    last_cbd = cbd_list[-1]
                    direction = (
                        -1 if len(cbd_list) > 1 and cbd_list[1] < cbd_list[0] else 1
                    )
                    while len(cbd_list) < len(cbd_tick_xs):
                        last_cbd += direction
                        cbd_list.append(last_cbd)

            # Create enhanced labels with CBD numbers and Bingham coordinates
            enhanced_labels = []
            valid_tick_positions = []

            print(f"INFO: Creating enhanced x-axis labels for {len(cbd_tick_xs)} ticks")

            for i, (tick_x, cbd) in enumerate(zip(cbd_tick_xs, cbd_list)):
                # Get navigation data for this CBD
                row = nav_df[nav_df["CBD"] == cbd]
                if not row.empty:
                    lat_bingham = row["LAT (bingham)"].values[0]
                    lon_bingham = row["LON (bingham)"].values[0]

                    # Format: CBD number on top line, coordinates on bottom lines
                    enhanced_labels.append(
                        f"{cbd}\n{lat_bingham:.3f}°\n{lon_bingham:.3f}°"
                    )
                    print(f"  CBD {cbd}: {lat_bingham:.3f}°, {lon_bingham:.3f}°")
                else:
                    print(f"  WARNING: CBD {cbd} not found in navigation data")
                    enhanced_labels.append(f"{cbd}\nN/A\nN/A")

                valid_tick_positions.append(tick_x)

            # Apply enhanced x-axis ticks and labels
            print(
                f"INFO: Setting {len(valid_tick_positions)} enhanced x-axis tick positions"
            )
            ax.set_xticks(valid_tick_positions)
            ax.set_xticklabels(enhanced_labels, rotation=0, fontsize=10, ha="center")

            # Add enhanced x-axis label with clear Bingham specification
            ax.set_xlabel(
                "CBD Number\nLatitude (Bingham)\nLongitude (Bingham)",
                fontsize=14,
                fontweight="bold",
            ...
            # Add subtle vertical reference lines for each CBD
            for x_pos in valid_tick_positions:
                ax.axvline(
                    x=x_pos,
                    color="lightblue",
                    linestyle=":",
            ...
            print(
                f"SUCCESS: Enhanced x-axis with {len(valid_tick_positions)} CBD positions and Bingham coordinates applied"
            )
        ...
        if nav_df is None:
            print("WARNING: No navigation data available for enhanced x-axis labels")
        if len(cbd_tick_xs) == 0:
            print("WARNING: No CBD tick positions available for enhanced x-axis labels")

    # === ENHANCED SURFACE/BED PLOTTING WITH DEDUPLICATED LEGEND ===
    legend_labels_used = set()  # Track used legend labels to prevent duplicates

    # Surface plotting with single legend entry
    if processor_ref and hasattr(processor_ref, "layers"):
        if processor_ref.layers["surface"]["picks"] is not None and np.any(
            np.isfinite(processor_ref.layers["surface"]["picks"])
        ):
            x_coords = np.arange(len(processor_ref.layers["surface"]["picks"]))

            # Plot surface with single legend entry
            surface_label = "Surface" if "Surface" not in legend_labels_used else ""
            plot_with_gaps(
                ax,
                x_coords,
                processor_ref.layers["surface"]["picks"],
                color="#009E73",
                linestyle="--",
                linewidth=3,
                label=surface_label,
            ...
            if surface_label:
                legend_labels_used.add("Surface")

        if processor_ref.layers["bed"]["picks"] is not None and np.any(
            np.isfinite(processor_ref.layers["bed"]["picks"])
        ):
            x_coords = np.arange(len(processor_ref.layers["bed"]["picks"]))

            # Plot bed with single legend entry (no matter how many segments)
            bed_label = "Bed" if "Bed" not in legend_labels_used else ""
            plot_with_gaps(
                ax,
                x_coords,
                processor_ref.layers["bed"]["picks"],
                color="#D55E00",
                linestyle="--",
                linewidth=3,
                label=bed_label,
            ...
            if bed_label:
                legend_labels_used.add("Bed")

    else:
        # Fallback to old parameter-based approach with deduplication
    ...
    # Ice thickness calculations - Updated for layer architecture
    # Get surface and bed data from processor layers or fallback
    if processor_ref and hasattr(processor_ref, "layers"):
        surface_data = processor_ref.layers["surface"]["picks"]
        bed_data = processor_ref.layers["bed"]["picks"]
    else:
        surface_data = surface_y_abs
        bed_data = bed_y_abs

    if (
        surface_data is not None
        and bed_data is not None
        and np.any(np.isfinite(surface_data))
        and np.any(np.isfinite(bed_data))
        and physics_constants is not None
    ):
        valid_surface = surface_data[np.isfinite(surface_data)]
        valid_bed = bed_data[np.isfinite(bed_data)]
    ...
    # === ENHANCED LEGEND WITH DEDUPLICATION ===
    # Get all legend entries and filter for unique labels only
    handles, labels = ax.get_legend_handles_labels()
    unique_legend = {}

    for handle, label in zip(handles, labels):
        if label and label not in unique_legend:
    ...
    if unique_legend:
        ...
        print(
            f"INFO: Created legend with {len(unique_legend)} unique entries: {list(unique_legend.keys())}"
        )
