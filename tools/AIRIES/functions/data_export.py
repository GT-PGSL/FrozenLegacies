...
def export_ice_measurements(
    ...
    if (
        ...
        transmitter_travel_time_us = np.zeros(n_pixels)  # Transmitter is at time zero
        transmitter_intensity = processor.transmitter_pulse_intensities
        transmitter_y_pixel = np.full(n_pixels, processor.transmitter_pulse_y_abs)
    else:
        transmitter_travel_time_us = np.full(n_pixels, np.nan)
        transmitter_intensity = np.full(n_pixels, np.nan)
        transmitter_y_pixel = np.full(n_pixels, np.nan)
    ...
    try:
        with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            headers = [
                ...
                "bed_depth_m",
                "ice_thickness_m",
                "transmitter_travel_time_us",
                "transmitter_intensity",
                "transmitter_y_pixel",
            ]
            ...
            for i in range(n_pixels):
                row = [
                    ...
                    f"{transmitter_travel_time_us[i]:.3f}"
                    if np.isfinite(transmitter_travel_time_us[i])
                    else "NaN",
                    f"{transmitter_intensity[i]:.1f}"
                    if np.isfinite(transmitter_intensity[i])
                    else "NaN",
                    f"{transmitter_y_pixel[i]:.1f}"
                    if np.isfinite(transmitter_y_pixel[i])
                    else "NaN",
                ]
                writer.writerow(row)
