...
def update(val):
    ...
    # Enable feature selection after adjusting the X-axis slider, but only once
    # per-frame calibration (reference line + Y/X grid lines) has been completed.
    if not can_click and y_axis_lines and x_axis_lines:
        can_click = True  # Allow feature selection after calibration + slider move
...
def next_ascope():
    ...
    # Reset per-frame calibration so the operator re-picks the reference line
    # and grid lines independently for each frame (handles oscilloscope drift).
    reference_line_y = None
    reference_line_y_orig = None
    setting_reference_line = False
    y_axis_points = []
    ...
    print(f"Moving to A-scope {current_scope} — please re-pick calibration (ref line + Y/X grid lines)")
...
def draw_y_axis_lines():
    ...
    # Anchor to the -60 dB reference line.
    # Pixels increase downward; dB increases upward. A reference line BELOW the
    # first Y-pick (reference_line_y > base_y) means the reference is at a LOWER
    # dB value than the first pick.
    # offset_divisions is continuous (no rounding) so the label at base_y is exact.
    # e.g. if the reference falls 0.5 divisions below a grid line, the grid line
    # is labelled -60 + 5 = -55 dB, not snapped to -60 or -50.
    offset_divisions = (reference_line_y - base_y) / avg_spacing
    base_db = -60 + offset_divisions * 10
...
def draw_x_axis_lines():
    ...
    avg_spacing = np.mean(np.diff(sorted_points))
    # Anchor to the 0 µs X reference line (main bang start).
    # Pixels increase rightward; time increases rightward. The first X-pick is
    # steps_from_ref major divisions to the RIGHT of the 0 µs reference line.
    if x_reference_line is not None:
        steps_from_ref = round((base_x - x_reference_line) / avg_spacing)
        base_us = steps_from_ref * 3  # 3 µs per major X division
    else:
        base_us = 3  # Fallback: assume first pick is at 3 µs if no reference set
